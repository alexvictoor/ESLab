...
public class LdbJniStore extends AbstractStore {
    ...
    public LdbJniStore(String path, Collection<Serializer> serializers) {
        ...
        try {
            eventDatabase = factory.open(new File(path, "events.leveldbjni"), options);
            schemaDatabase = factory.open(new File(path, "schemas.leveldbjni"), options);
        } catch (IOException e) {
        ...
        Runtime.getRuntime().addShutdownHook(new Thread() {
            public void run() {
                try {
                    ...
                    System.out.println("Error closing LevelDB file");
                }
            }
